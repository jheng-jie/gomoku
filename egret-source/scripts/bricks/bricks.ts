...
export class BricksPlugin implements plugins.Command {
    ...
    async onFile(file: plugins.File) {
        ...
        if (filename == 'manifest.json') {
            ...
            let result = content.replace(/RES\.loadConfig\("resource\/default\.res\.json", "resource\/"\)/gm, 'RES.loadConfig("GameRes://dist/resource/default.res.json", "GameRes://dist/resource/")');
            result = result.replace(/eui\.Theme\("resource\/default\.thm\.json", _this\.stage\)/gm, 'eui.Theme("GameRes://dist/resource/default.thm.json", _this.stage)');
