	...
	private createUI(){
		...
		Object.defineProperty(this,'faceIndex',{
            ...
            set: function(i:number){
                ...
                this.faceEvent.animate = TweenMax.to(
                    this.face,1, {
                        x:i*-this.stageW,
	...
	public onFace(e){
        ...
        switch(e.type){
            ...
            case "touchEnd":
				if(event.hold){
				    let x = this.face.x;
					let index = Math.round(Math.abs(x)/this.stageW);
                    let max = (this.stageW*5+this.stageW*.5);
                    if(x > 0){
